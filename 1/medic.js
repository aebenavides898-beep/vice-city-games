...
function upcomingMonToSat(days = 14) {
  const out = []
  const now = new Date()
...
function pickDateFriendly(cb) {
  ...
  console.log("\nPick a date (Mon–Sat only). You can type:")
  console.log("- A number from the list below")
  console.log("- today / tomorrow")
  console.log("- A date like 2025-11-04")
  ...
  ask("Your choice", raw => {
    ...
    if (Number.isFinite(n) && n >= 1 && n <= upcoming.length) {
      return cb(fmtDate(upcoming[n-1]))
    }

    const manual = toDate(raw)
    if (manual && isMonToSat(manual)) {
      return cb(fmtDate(manual))
    }
...
function freeSlotsFor(doctorIndex, dateStr) {
  ...
  for (const t of timeSlots) {
    if (!occupied.has(slotKey(doctorIndex, dateStr, t))) free.push(t)
  }
  return free
}
...
function pickTimeFriendly(doctorIndex, dateStr, cb) {
  ...
  if (free.length === 0) {
    console.log("\nNo free time slots on " + dateStr + " for this doctor.")
    return cb(null)
  }
  console.log("\nAvailable time slots on " + dateStr + ":")
  for (let i = 0; i < free.length; i++) {
    console.log((i+1) + ") " + free[i])
  }
  ask("Choose a time by number", txt => {
    const n = parseInt(txt, 10)
    if (!Number.isFinite(n) || n < 1 || n > free.length) {
      ...
      pickTimeFriendly(di, dateStr, timeStr => {
        ...
        ask("Enter patient name", patient => {
          ...
          ask("Confirm? (1 yes / 2 no)", ok => {
            ...
            appointments.set(id, rec)
            occupied.add(slotKey(di, dateStr, timeStr))
...
function viewAppointments() {
  ...
  if (appointments.size === 0) { console.log("No appointments."); return mainMenu() }
